...
A robotic hand that mirrors your hand (finger bend and wave of wrist) in real-time through a webcam. 
...
- Computes hand tilt from the wrist→palm vector to drive a 6th servo that rotates the whole hand — tilt your hand and it waves.
- Maps each finger measurement to its servo's calibrated range and drives 6 servos so the robot hand mirrors yours
...
- **Hardware** — ELEGOO Uno R3, 5× micro servos in LewanSoul/Hiwonder uHand + MG996R wrist servo, external 5V supply, breadboard

## Per frame Pipeline 

```
webcam → MediaPipe (21 landmarks) → per-finger bend angle + hand tilt
        → map to servo range → smooth → command over serial  
        → Arduino firmware → servos → hand
```
